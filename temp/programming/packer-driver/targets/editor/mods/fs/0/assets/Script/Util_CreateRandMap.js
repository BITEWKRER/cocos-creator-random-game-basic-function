System.register(["cce:/internal/code-quality/cr.mjs", "cc", "./Util_SimpleMath"], function (_export, _context) {
  ...
  var _reporterNs, _cclegacy, Util_SimpleMath, Util_CreateRandMap, _crd;
  ...
  _export("Util_CreateRandMap", void 0);
  ...
  return {
    ...
    execute: function () {
      ...
      _export("Util_CreateRandMap", Util_CreateRandMap = class Util_CreateRandMap {
        /*
          ...
          */
        static initRoomList(room_number, max_width, max_height, min_width, min_height) {
          let rooms = []; //在一个圆形区域内生成若干个大小不同的房间

          const r = 100;

          for (let i = 0; i < room_number * 4; i++) {
            const room_width = min_width + Math.floor(Math.random() * (max_width - min_width));
            const room_height = min_height + Math.floor(Math.random() * (max_height - min_height));
            const room_raid = Math.random() * Math.PI * 2;
            const room_x = Math.floor(Math.cos(room_raid) * Math.random() * r);
            const room_y = Math.floor(Math.sin(room_raid) * Math.random() * r);
            let room_local = {
              x: room_x,
              y: room_y,
              w: room_width,
          ...
          for (let i = 0; i < rooms.length; i++) {
            let searching = true;
            let raid = (_crd && Util_SimpleMath === void 0 ? (_reportPossibleCrUseOfUtil_SimpleMath({
              error: Error()
            }), Util_SimpleMath) : Util_SimpleMath).getAngleByTowPoint(rooms[i].x, rooms[i].y, mid.x, mid.y) / 180 * Math.PI;
            let speed = 5;
            ...
            while (searching) {
              ...
              if ((_crd && Util_SimpleMath === void 0 ? (_reportPossibleCrUseOfUtil_SimpleMath({
                ...
                error: Error()
              }), Util_SimpleMath) : Util_SimpleMath).getDistanceByTowPoint(rooms[i].x, rooms[i].y, mid.x, mid.y) <= speed) {
                searching = false;
              }
            ...
            if (i % 4 == 0) MRoom.push(rooms[i]);else VRoom.push(rooms[i]);
          }
        ...
        static Bowyer(poinset, index, triangle) {
          //初次执行先添加超级三角形
          if (index == 0) {
            poinset.push({
              x: -30000,
          ...
          for (let i = triangle.length - 1; i >= 0; i--) {
            //计算三角形外接圆的圆心和周长
            const c_center = (_crd && Util_SimpleMath === void 0 ? (_reportPossibleCrUseOfUtil_SimpleMath({
              error: Error()
            }), Util_SimpleMath) : Util_SimpleMath).getTriangleCircleCenter(poinset[triangle[i][0]].x, poinset[triangle[i][0]].y, poinset[triangle[i][1]].x, poinset[triangle[i][1]].y, poinset[triangle[i][2]].x, poinset[triangle[i][2]].y);
            const c_r = (_crd && Util_SimpleMath === void 0 ? (_reportPossibleCrUseOfUtil_SimpleMath({
              error: Error()
            }), Util_SimpleMath) : Util_SimpleMath).getTriangleCircleRadius(poinset[triangle[i][0]].x, poinset[triangle[i][0]].y, poinset[triangle[i][1]].x, poinset[triangle[i][1]].y, poinset[triangle[i][2]].x, poinset[triangle[i][2]].y); //判断点是否在外接圆内
          ...
          for (let i = 0; i < edge.length - 1; i++) {
            ...
            for (let j = i + 1; j < edge.length; j++) {
              if (edge[i].st == edge[j].st && edge[i].ed == edge[j].ed || edge[i].ed == edge[j].st && edge[i].st == edge[j].ed) {
                edge[j] = -1;
                is_re = true;
              }
            }
        ...
        static createConnectedRoad(rooms, vrooms) {
          let roadRects = [];
          let triangle = [];
          ...
          let tree = (_crd && Util_SimpleMath === void 0 ? (_reportPossibleCrUseOfUtil_SimpleMath({
            error: Error()
          ...
          rooms.forEach((room_i, index) => {
            if (room_i.to.length > 0) {
              for (let i = 0; i < room_i.to.length; i++) {
                ...
                let roadRect = []; //由于房间不重叠 所以不会出现 horizon 和 vertical 都为0的情况
        ...
        static createTerrains(rooms) {}

        static createTerrain(room, terrainInfo) {
          let terrains = [];
          ...
          for (let i = 0; i < terrainInfo.length; i++) {
            let S = terrainInfo[i].area * room.w * room.h;
            let p = 1;
            let areanumber = 1 + Math.ceil(Math.random() * 5);
            ...
            for (let j = 0; j < areanumber; j++) {
              ...
              let s_j = S * p_j;
              let r = Math.ceil(Math.sqrt(s_j / Math.PI));
              let rp = {
