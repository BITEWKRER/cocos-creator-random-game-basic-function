System.register(["cc"], function (_export, _context) {
  ...
  var _cclegacy, _decorator, Component, resources, JsonAsset, _dec, _class, _crd, ccclass, mtl, Intimacy, Person, UntilRelationship;
  ...
  return {
    ...
    execute: function () {
      ...
      ccclass = _decorator.ccclass; //性格种类

      mtl = ["正常", "冷漠", "热情"];
      Intimacy = ["陌生", "熟识", "好友"];

      _export("Person", Person = /*#__PURE__*/function () {
        //姓名
        //性别
        //死因描述
        //职业
        ...
        //线索
        //性格
        function Person(name, age, sex, belongs) {
        ...
        _proto.intToChinese = function intToChinese(str) {
          str = str + "";
        ...
        _proto.setIntimacy = function setIntimacy(stage) {
          var _this = this;
          ...
          this.Intimacy = stage;
          resources.load("JSON/clue", JsonAsset, function (err, clues) {
            var data = JsonAsset.deserialize(clues).json;
            var len = data[_this.mettle][_this.Intimacy].length;

            var clue = data[_this.mettle][_this.Intimacy][Math.floor(Math.random() * len)];

            return _this.setClue(clue);
          });
        };

        _proto.setClue = function setClue(clue) {
          return this.clue = clue;
        };

        _proto.getMettle = function getMettle() {
          return this.mettle;
        };

        _proto.getIntimacy = function getIntimacy() {
          return this.Intimacy;
        };

        _proto.getClue = function getClue() {
          return this.clue;
        };

        _proto.setDeadDis = function setDeadDis(dis) {
          this.deadDis = dis;
        };

        _proto.setDeadSts = function setDeadSts(sts) {
          this.deadSts = sts;
        };

        _proto.getDeadSts = function getDeadSts() {
          return this.deadSts;
        };

        _proto.getDeadDis = function getDeadDis() {
          return this.deadDis;
        };
        ...
        _proto.setOccupation = function setOccupation(occupation) {
          this.occupation = occupation;
        };

        _proto.getOccupation = function getOccupation() {
        ...
        _proto.getBelong = function getBelong() {
          return this.belongs;
        };
      ...
      _export("UntilRelationship", UntilRelationship = (_dec = ccclass("UntilRelationship"), _dec(_class = /*#__PURE__*/function (_Component) {
        _inheritsLoose(UntilRelationship, _Component);

        function UntilRelationship() {
          return _Component.apply(this, arguments) || this;
        }

        UntilRelationship.getTown = function getTown(members) {
          if (members > 7) {
            return false;
          } //姓
          ...
          for (var i = 0; i < members; i++) {
            ...
            for (var _j = 0; _j < familyMembers; _j++) {
            ...
            for (var _i = 0, _j2 = 0; _i < queue.length; _i++) {
              if (queue[_i].getAge() != -1) {
                result[_j2] = queue[_i];
                _j2++;
              }
            } // dead rand
            ...
            if (result.length > lim) {
              for (var _i2 = 0; _i2 < result.length; _i2++) {
                ...
                if (nameFlag == 1) {
                  result[_i2].setDeadSts(true);

                  if (result[_i2].getAge() <= 18) {
                    result[_i2].setDeadDis("夭折");
                  } else if (result[_i2].getAge() <= 60) {
                    result[_i2].setDeadDis("早逝");
                  } else if (result[_i2].getAge() <= 75) {
                    nameFlag = this.randNum(4) % 2;

                    if (nameFlag == 1) {
                      result[_i2].setDeadDis("客死");
                    } else {
                      result[_i2].setDeadDis("遇难");
                    }
                  } else {
                    result[_i2].setDeadDis("正寝");
                  }
        ...
        UntilRelationship.setOccupation = function setOccupation(town_family, members) {
          var low = [["酒肆", "掌柜", "店小二", "杂役"], ["客栈", "店老大", "跑堂", "庖丁"], ["医坊", "医师", "学徒", "杂役"], ["铁匠铺", "店老大", "铁匠", "学徒"], ["茶馆", "掌柜", "店小二", "杂役"], ["裁缝店", "掌柜", "店小二", "裁缝"], ["商铺", "掌柜", "账房", "店小二"]];
          var rad; // 基础店家分配
          // let cnt = 1;
          ...
          for (var i = 0; i < members; i++) {
            ...
            for (var j = 0; j < town_family[i].fam.length; j++) {
              if (town_family[i].fam[j].getSex() == "man") {
        ...
        UntilRelationship.getAName = function getAName(name, second, sex) {
          return name + this.getARandomSecondName(second, sex);
        };

        UntilRelationship.setPerporty = function setPerporty(tmp, name, age, sex) {
          tmp.setAge(age);
          tmp.setName(name);
          tmp.setSex(sex);
        };

        UntilRelationship.randNum = function randNum(num) {
          return Math.floor(Math.random() * num);
        };

        UntilRelationship.getARandomSecondName = function getARandomSecondName(second, type) {
        ...
        UntilRelationship.getARandomFirstName = function getARandomFirstName(first) {
        ...
        UntilRelationship.getAllFirstName = function getAllFirstName() {
        ...
        UntilRelationship.getAllSecondName = function getAllSecondName() {
          var myMing = {
            manSingle: [""],
            manDouble: [""],
            womanSingle: [""],
        ...
        UntilRelationship.getName = function getName(first, second, firstType, secondType, sex) {
          var firstName = "";
          var secondName = "";
          var num = 0;
